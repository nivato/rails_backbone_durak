class Rules
  ...
  def select_higher_cards_of_same_suit(attackers_card)
    ...
    cards_of_same_suit = get_cards.collect do |card|
      card if Card.suit(card) == Card.suit(attackers_card)
    end
    ...
    unless cards_of_same_suit == []
      cards_of_higher_rank = cards_of_same_suit.collect do |card|
        card if Card.rank_number(card) > Card.rank_number(attackers_card)
      end
    end
  ...
  def get_cards_of_same_ranks(cards_on_table)
    cards_of_same_rank = []
    table_ranks = cards_on_table.collect{|card| Card.rank_number(card)}
    table_ranks = table_ranks.uniq
    get_cards.each do |card|
      if table_ranks.include? Card.rank_number(card)
  ...
  def select_lowest_card(cards_in_hand)
    ...
    cards_in_hand.each do |card|
      lowest_card = card if Card.rank_number(card) == min_rank
    end
    return lowest_card
  end
